...
> **[실측 2026-08-13 · 로컬 윈도우]** `mediapipe==1.0.0` 설치 결과 **핀 변경 0건**.
> `numpy 2.0.2` · `torch 2.11.0+cu128` · `torchvision 0.26.0+cu128` · `ultralytics 8.4.76` 모두 그대로였고,
> 추가만 7개(`absl-py` `cffi` `flatbuffers` `mediapipe` `opencv-contrib-python` `pycparser` `sounddevice`).
> 설치 후에도 `torch.cuda.is_available()` 정상. **다만 이건 윈도우 로컬 결과이고 코랩은 리눅스라
> 의존성 해석이 다를 수 있다 — CONVENTIONS 규칙 3 에 따라 핀은 코랩 확인 후에 확정한다.**
> 지금은 계속 **핀 없이** 둔다.
...
- [x] 런타임을 새로 시작하고 셀 1·2·3·**4** 후 **셀 8 만** 돌려도 `runs.json` 으로 동작한다
  > **[정정 2026-08-13]** 원문은 "셀 1·2·3 후 셀 8만" 이었으나 **셀 4가 빠지면 동작하지 않는다.**
  > 셀 8이 셀 4에서 정의한 `build_model` / `load_runs` 를 쓰기 때문이다(`RuntimeError` 로 명확히 안내됨).
  > 셀 4는 **정의만 하고 학습하지 않아 1초도 안 걸리므로** 복구 비용은 사실상 같다.
  > 셀 8을 자체완결로 만들면 셀 9까지 같은 코드를 중복해야 해서, **문서를 현실에 맞추는 쪽**을 택했다.
  > 실측 확인: 새 프로세스에서 셀 1·2·3·4 → 8 정상 동작, 셀 1·2·3 → 8 은 `RuntimeError`.
- [x] `02` 셀 3 이 웹캠 거부 상황에서도 **예외 없이** 폴백 이미지로 진행된다
  > 로컬(웹캠 캡처 불가 = 거부와 같은 경로)에서 확인. 3단 폴백이 전부 돌아 샘플 이미지 640×960 확보,
  > 셀 4 가 손 2개(각 21점, Left/Right)·셀 5 가 사람 1명(17 키포인트) 검출까지 도달.
- [x] `02` 셀 2 가 mediapipe 설치 후 numpy/torch 버전을 출력한다
  > **[버그 2건 수정 2026-08-13]** 원래 코드는 `__import__(mod).__version__` 으로 버전을 봤는데,
  > 1. `ultralytics` 를 import 하면 `cv2` 가 로드돼 DLL 이 잠기고, **윈도우에서는 pip 이 opencv 를
  >    교체하지 못해 mediapipe 설치가 통째로 실패**했다(`WinError 5`). 리눅스는 로드된 `.so` 교체가
  >    되므로 코랩에서는 안 터진다 — 로컬에서만 드러나는 문제.
  > 2. 더 심각한 쪽: **pip 이 방금 덮어썼어도 이미 import 된 모듈은 캐시된 옛 버전을 돌려준다.**
  >    즉 덮어쓰기가 실제로 일어나도 "이상 없음" 이라고 잘못 보고한다 — 이 셀의 존재 이유가 무너진다.
  >    (코드 주석은 "pip 이 아는 값을 함께 본다" 였지만 실제로는 그러지 않았다.)
  >
  > → `importlib.metadata.version()` 으로 **import 없이 디스크에서** 읽도록 고쳤다.
  > pip 실패 시 "이상 없음" 이 안전을 뜻하지 않는다는 경고도 추가.
  > 원래 실패 조건(mediapipe·opencv-contrib 제거 후 재설치)을 재현해 수정 전 실패 → 수정 후 성공 확인.
