@echo off
REM ============================================================
REM setup_env.bat - session4 로컬 conda 환경 자동 셋업 (윈도우)
REM ============================================================
...
set "ENV_NAME=ai_cop_session4"

REM 코랩과 Python 버전을 맞춘 값(session2 에서 확인).
set "PY_VERSION=3.12"

REM 스크립트 위치(session4\) 로 이동
cd /d "%~dp0"
...
REM 활성화가 실제로 됐는지 확인한다. 여기서 멈추지 않으면 다음 단계의 pip 가
REM base 환경에 torch 를 설치해 버린다(아나콘다 기본 환경 오염). 반드시 중단할 것.
if not "%CONDA_DEFAULT_ENV%"=="%ENV_NAME%" (
  echo [오류] conda activate 에 실패했습니다. ^(현재 환경: "%CONDA_DEFAULT_ENV%"^)
  echo        base 환경을 오염시키지 않기 위해 여기서 중단합니다.
  echo.
  echo        해결: "Anaconda Prompt" 를 열어 다시 실행하거나,
  echo              일반 cmd 라면 "conda init cmd.exe" 를 한 번 실행하고 새 창에서 재시도하세요.
  exit /b 1
)
...
if exist requirements.txt (
  ...
  echo [경고] requirements.txt 가 없습니다. session4\ 안에서 실행했는지 확인하세요.
)
...
echo             jupyter notebook notebooks\01_train_lab.ipynb
echo (참고) mediapipe 는 여기서 설치하지 않습니다 - 02_pose_demo 노트북 셀 2에서만 설치합니다.
echo (참고) GPU 없는 로컬은 실험 1회가 수 분씩 걸립니다 - 코랩 T4 권장.
endlocal
